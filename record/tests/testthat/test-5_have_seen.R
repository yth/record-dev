...
test_that("have seen twice", {
	...
	add_val(s1)
	add_val(s2)
	expect_equal(have_seen(s1), T)
	expect_equal(have_seen(s2), T)
	close_db()
})

test_that("have not seen not added val", {
	...
	expect_equal(have_seen(s1), T)
	expect_equal(have_seen(s2), T)
	expect_equal(have_seen(s3), F)
	close_db()
})

test_that("have seen 100 int vals", {
  open_db_for_write("seen-100-vals.txt")
  add_val(1:100)
  expect_equal(have_seen(1:100), TRUE)
  close_db()
})
